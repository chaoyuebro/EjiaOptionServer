...
bool IsInClearCacheTime(int& iMonth)
{
	...
	localtime_r(&now, &tms);
	iMonth = tms.tm_mon;
	if (tms.tm_hour == 20 && tms.tm_min <= 30)
	{
		int iYmd = (tms.tm_year + 1900) * 10000 + (tms.tm_mon + 1) * 100 + tms.tm_mday;
...
//新交易日夜盘开始之前的19:30到20:00之间清理当天的tick、1分钟、5分钟和日线的缓存队列。
void ClearCache()
{
	...
	if (now - s_LastClearCacheTime >= 23 * 60 * 60 && IsInClearCacheTime(iMonth))
	{
		...
		//清理tick去重队列 - 添加异常保护和强制内存释放
		try {
			g_lockUniqueTick.lock();
			g_Logger->info("清理去重队列，当前大小：set={} list={}", 
						  g_setUniqueTick.size(), g_listUniqueTick.size());
			// 强制释放内存：使用swap技巧
		...
		{
			g_lockUniqueTick.unlock();
			g_Logger->error("清理去重队列时发生异常");
		}
		//清理近两百条全量tick缓存 - 添加异常保护和强制内存释放
		try {
			g_lock200QuanTick.lock();
			// 强制释放内存：使用swap技巧
		...
		{
			g_lock200QuanTick.unlock();
			g_Logger->error("清理200条全量tick数据时发生异常");
		}
		//清理当天未压缩和已压缩的增量tick缓存 - 添加异常保护和强制内存释放
		try {
			g_lockCompressTick.lock();
			// 强制释放内存：使用swap技巧
		...
		{
			g_lockCompressTick.unlock();
			g_Logger->error("清理压缩tick数据时发生异常");
		}
		//清理当天1分钟线的时间和根数 - 添加异常保护和强制内存释放
		try {
		...
		{
			g_lock1MinKlineCount.unlock();
			g_Logger->error("清理1分钟线计数时发生异常");
		}
		//清理当天1分钟线 - 添加异常保护和强制内存释放
		try {
			g_lock1MinKline.lock();
			// 强制释放内存：使用swap技巧
		...
		{
			g_lock1MinKline.unlock();
			g_Logger->error("清理1分钟线数据时发生异常");
		}
		//清理当天5分钟线的时间和根数 - 添加异常保护和强制内存释放
		try {
		...
		{
			g_lock5MinKlineCount.unlock();
			g_Logger->error("清理5分钟线计数时发生异常");
		}
		//清理当天5分钟线 - 添加异常保护和强制内存释放
		try {
			g_lock5MinKline.lock();
			// 强制释放内存：使用swap技巧
		...
		{
			g_lockDayKline.unlock();
			g_Logger->error("清理日线数据时发生异常");
		}
		//清理其他数据 - 添加异常保护和强制内存释放
		try {
			//清理已上传过Native数据的合约id - 强制释放内存
			set<OptionID>().swap(g_setUploadNativeQuoteId);
			//清除当天已发送短信的总条数
			CTcpPullClientImpl::s_iDXTotalNum = 0;

			//删除本地三天之前的历史数据 - 使用带超时的线程执行
			g_Logger->info("{}->启动历史数据清理线程（5分钟超时）", __FUNCTION__);
			...
			std::thread cleanThread([&cleanFinished]() {
				try {
					...
					g_Logger->error("历史数据清理线程异常: {}", e.what());
					cleanFinished = true;
				}
				catch (...) {
					g_Logger->error("历史数据清理线程发生未知异常");
					cleanFinished = true;
				}
			});
			...
			while (!cleanFinished) {
				...
				if (elapsed >= timeoutSeconds) {
					timeout = true;
					g_Logger->warn("历史数据清理超时（{}秒），强制结束线程", timeoutSeconds);
					break;
				}
			}
			...
			if (timeout) {
				// 超时了，分离线程让它自己结束
				cleanThread.detach();
				g_Logger->warn("历史数据清理线程已分离，将在后台继续执行");
			}
			else {
				// 正常完成，等待线程结束
				...
				g_Logger->info("历史数据清理线程正常完成");
			}
		}
		catch (...)
		{
			g_Logger->error("清理其他数据时发生异常");
		}
		
		g_Logger->info("{}->内存数据缓存清理完成", __FUNCTION__);
...
void listDirAndClean(const string& strPath, int iYmd, int& deletedFiles, int& emptyDirs)
{
	...
	while ((ent = readdir(pDir)) != NULL)
	{
		...
		else
		{
			...
			// 处理文件名字里面包含下划线_  比如: CFFEX_TF_09_20190531.5m
			...
			if (shouldDelete)
			{
				...
				{
					g_Logger->info("已删除 {} 个过期文件...", deletedFiles);
				}
			}
		}
	}
...
void CleanHistoryData()
{
	...
	g_Logger->info("{}->开始清理三天之前的本地历史数据文件", __FUNCTION__);
	...
	g_Logger->info("开始扫描dig目录: {}, 截止日期: {}", (LPCSTR)strDigPath, iYmd);
	...
	g_Logger->info("dig目录扫描完成，删除文件数: {}, 删除空目录数: {}, 耗时: {}ms", deletedDigFiles, emptyDigDirs, digDuration);

	g_Logger->info("开始扫描original目录: {}", (LPCSTR)strOriginalPath);
	...
	g_Logger->info("original目录扫描完成，删除文件数: {}, 删除空目录数: {}, 耗时: {}ms", deletedOriginalFiles, emptyOriginalDirs, originalDuration);

	auto endTime = std::chrono::steady_clock::now();
	auto totalDuration = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - startTime).count();
	g_Logger->info("历史数据清理完成, 总共删除文件数: {}, 总共删除空目录数: {}, 总耗时: {}ms",
